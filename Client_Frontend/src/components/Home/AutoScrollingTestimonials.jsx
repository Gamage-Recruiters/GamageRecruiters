...
const AutoScrollingTestimonials = () => {
  ...
  // Sample testimonials data - this would typically be imported from testimonials.js
  const testimonials = [
    {
  ...
  useEffect(() => {
    ...
    // Only set up scrolling if there's overflow content
    ...
    let animationRef = { current: requestAnimationFrame(scroll) };
    ...
    <div className="bg-gradient-to-b from-gray-50 to-white py-24">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div 
          ...
          className="flex overflow-x-hidden pb-4 gap-6"
        >
          {testimonials.map((testimonial, index) => (
            <div 
              key={index} 
              className="flex-shrink-0 w-full md:w-1/3 lg:w-1/3 bg-white rounded-xl shadow-md p-8 transform transition duration-500 hover:-translate-y-2 hover:shadow-xl border border-gray-100"
            >
              ...
              <div 
                className="flex items-center text-indigo-600 font-medium mt-4" 
        ...
        <button
        onClick={handleClick}
        className="px-6 py-3 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition duration-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
        >
